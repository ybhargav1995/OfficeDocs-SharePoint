...
5. Select **Allow only specific domains** to create an allowlist (must be a subset of the organization's allowlist), or to block only the domains you specify, select **Block specific domains**. You won't be able to use the **Block specific domains** feature if your oranization has already set up a allowlist.
